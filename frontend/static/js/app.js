...
function bindEventListeners() {
    elements.workflowSelect.addEventListener("change", handleWorkflowChange);
...
async function fetchWorkflows() {
    try {
        ...
        console.error(error);
        setFormFeedback("error", "Unable to load workflows. Please refresh the page.");
    }
}
...
function handleWorkflowChange(event) {
    ...
    state.selectedWorkflowId = workflowId || null;
    clearFormFeedback();
...
function renderWorkflowFields(workflow) {
    elements.dynamicFields.innerHTML = "";
...
async function handleFormSubmit(event) {
    ...
    if (!workflowId) {
        setFormFeedback("error", "Please select a workflow before submitting.");
        return;
    }

    const workflow = state.workflows[workflowId];
    if (!workflow) {
        setFormFeedback("error", "Selected workflow metadata not found.");
        return;
    }
    ...
    try {
        ...
        if (run) {
            run.inputs = inputs;
            addOrUpdateRun(run);
            setFormFeedback("success", `${workflow.name} submitted successfully.`);
            selectRun(run.id);
        }
        ...
        elements.submitBtn.disabled = false;
        elements.submitBtn.textContent = "Submit Workflow";
    }
}
...
function renderRunList() {
    ...
    if (runs.length === 0) {
        container.innerHTML = `
            <div class="empty-state">
                <h3>No runs yet</h3>
                <p>Submit a workflow to see it appear here.</p>
            </div>
        `;
        return;
    }
    ...
    runs.forEach((run) => {
        ...
        title.textContent = run.workflow_name || run.workflow_id;
    ...
    switch (type) {
        ...
        case "error":
            return {
                timestamp,
                title: "Error",
                message: payload.message || "Workflow error",
                type,
                groupId,
                raw: payload
            };
...
function buildEventsContent(run) {
    ...
    if (!store || activeIds.length === 0) {
        ...
        if (status === "running" || status === "queued" || status === "cancelling") {
            ...
            placeholder.textContent = "Workflow encountered an error.";
        } else if (store && store.terminalStatus === "cancelled") {
            placeholder.textContent = "Workflow cancelled.";
        } else if (status === "completed") {
            placeholder.textContent = "Workflow completed.";
        } else {
