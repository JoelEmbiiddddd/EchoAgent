"""AgentZ Workflow Manager GUI (gui2).

Provides a bright, three-column interface for selecting workflows,
...
@dataclass
class WorkflowField:
    """Metadata describing a configurable workflow input field."""
...
def discover_example_workflows() -> Dict[str, Dict[str, Any]]:
    """Discover available workflows by scanning example scripts for workflow imports.

    - Parses Python files in `examples/` without importing them (avoids executing code)
    - Detects which workflow classes they reference (e.g., DataScientistWorkflow)
    - Builds workflow entries only for those with a corresponding example
    - Attaches `example_module` dynamically to preserve runtime behavior
    """
    root_dir = Path(__file__).resolve().parent.parent
    examples_dir = root_dir / "examples"

    # Map imported workflow class name -> example module path
    ...
    # Base workflow catalog (static metadata); we only expose entries with examples
    catalog: Dict[str, Dict[str, Any]] = {
        "data_scientist": {
            "name": "Data Scientist",
            "description": "Iterative data science workflow with multiple specialized agents.",
            "config": "workflows/configs/data_science.yaml",
        ...
        "web_searcher": {
            ...
            "config": "workflows/configs/web_searcher.yaml",
            "primary_input": "prompt",
            "fields": [
                WorkflowField(
                    id="prompt",
        ...
        "simple": {
            ...
            "config": "workflows/configs/simple.yaml",
            "primary_input": "prompt",
            "fields": [
                WorkflowField(
                    id="prompt",
        ...
        "vanilla_chat": {
            ...
            "config": "workflows/configs/vanilla_chat.yaml",
            "primary_input": "prompt",
            "fields": [
                WorkflowField(
                    id="prompt",
    ...
    class_name_map = {
        "data_scientist": "DataScientistWorkflow",
        "web_searcher": "WebSearcherWorkflow",
        "simple": "SimpleWorkflow",
        "vanilla_chat": "VanillaChatWorkflow",
    }
    ...
    for pid, meta in catalog.items():
        ...
        if example_mod:
            ...
            workflows[pid] = meta_with_example

    return workflows


# Build available workflows list from examples
AVAILABLE_WORKFLOWS: Dict[str, Dict[str, Any]] = discover_example_workflows()


def generate_run_id() -> str:
    """Create a short unique identifier for workflow runs."""
...
def extract_result_text(result: Any) -> str:
    """Best-effort conversion of workflow result objects to text."""
    if result is None:
        return ""
...
def run_workflow_thread(run_id: str) -> None:
    """Execute a workflow run in a background thread."""
    run_entry = workflow_runs.get(run_id)
    if not run_entry:
        return

    workflow_def = AVAILABLE_WORKFLOWS[run_entry["workflow_id"]]
    ...
    try:
        ...
        raw_config = workflow_def["config"]
        ...
        example_mod = workflow_def.get("example_module")
        if example_mod:
            # Run example module to preserve dynamic behaviors (e.g., timestamps)
            thread_local = getattr(run_workflow_thread, "_tls", None)
            if thread_local is None:
                thread_local = threading.local()
                setattr(run_workflow_thread, "_tls", thread_local)
            thread_local.run_id = run_id

            from workflows import base as base_mod
            from echoagent.agent import tracker as tracker_mod

            orig_init = base_mod.BaseWorkflow.__init__
            orig_run_sync = base_mod.BaseWorkflow.run_sync
            ...
            def _patched_start_printer(self):
                ...
                if self._printer is None:
                    ...
                    if current_id and current_id in workflow_runs:
                        self._printer = workflow_runs[current_id]["printer"]
                    else:
            ...
            def _patched_init(self, config):  # type: ignore[override]
                ...
                current_id = getattr(thread_local, "run_id", None)
                if current_id and current_id in workflow_runs:
                    workflow_runs[current_id]["workflow_instance"] = self

            def _patched_run_sync(self, *args, **kwargs):
                out = orig_run_sync(self, *args, **kwargs)
                current_id = getattr(thread_local, "run_id", None)
                if current_id and current_id in workflow_runs:
                    workflow_runs[current_id]["result"] = extract_result_text(out)
                return out

            # Apply patches
            tracker_mod.RuntimeTracker.start_printer = _patched_start_printer  # type: ignore[attr-defined]
            base_mod.BaseWorkflow.__init__ = _patched_init  # type: ignore[attr-defined]
            base_mod.BaseWorkflow.run_sync = _patched_run_sync  # type: ignore[attr-defined]
            ...
            try:
                ...
                base_mod.BaseWorkflow.__init__ = orig_init
                base_mod.BaseWorkflow.run_sync = orig_run_sync

            result = workflow_runs.get(run_id, {}).get("result")
        else:
            raise RuntimeError("No example module found for selected workflow. Example-driven runs only.")
...
@app.route("/")
def index() -> str:
    """Render the main workflow manager interface."""
    return render_template("index.html")


@app.route("/api/workflows", methods=["GET"])
def list_workflows():
    """Return available workflows and their input schemas."""
    payload = []
    ...
    return jsonify({"workflows": payload})
...
def list_runs():
    ...
    for run_id, data in sorted(
        workflow_runs.items(), key=lambda item: item[1]["created_at"], reverse=True
    ):
        runs.append(
            {
                "id": run_id,
...
def get_run(run_id: str):
    ...
    return jsonify(
        {
            "id": run_id,
            "workflow_id": run_entry["workflow_id"],
            "workflow_name": run_entry["workflow_name"],
...
def create_run():
    """Create a new workflow run with provided inputs."""
    ...
    workflow_runs[run_id] = run_entry
...
def stop_run(run_id: str):
    ...
    workflow_instance = run_entry.get("workflow_instance")
    if workflow_instance and hasattr(workflow_instance, "stop"):
        try:
        ...
        while True:
            ...
            if update:
                payload = {
                    ...
                    "run_status": workflow_runs[run_id]["status"],
                }
                ...
                status = workflow_runs[run_id]["status"]
                if status in {"completed", "error", "cancelled"}:
                    break
                time.sleep(0.1)

        # Final snapshot
        final = workflow_runs.get(run_id)
        if final:
            summary = {
                "event": "summary",
                "payload": {
...
if __name__ == "__main__":
    ...
    for workflow_id, details in AVAILABLE_WORKFLOWS.items():
        print(f"  - {details['name']} ({workflow_id})")
