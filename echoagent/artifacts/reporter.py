"""Shared data models and RunReporter facade for workflow runs."""
...
class RunReporter:
    ...
    def set_final_result(self, result: Any) -> None:
        """Store workflow result for later persistence."""
        with self._lock:
    ...
    def record_panel(
        ...
        *,
        ...
        with self._lock:
            # Create panel record
            from echoagent.artifacts.artifact_writer import _utc_timestamp
            record = PanelRecord(
                title=title,
                content=content,
                border_style=border_style,
