...
def get_env_with_prefix(base_name: str, prefix: str = "DR_", default: str = None) -> str:
    ...
    Args:
        base_name: The base name of the environment variable (e.g., "OPENAI_API").
...
def _load_openai_env() -> Dict[str, str]:
    ...
    missing: List[str] = []
    if not openai_url:
        missing.append("OPENAI_URL/openai_url")
    if not api_key:
        missing.append("OPENAI_API/api")
    if not model:
...
def get_api_key_from_env(provider: str) -> str:
    ...
    if not api_key:
        raise ValueError(
            "API key not found. Set OPENAI_API/api in environment or .env file."
        )
...
def resolve_config(spec: Union[str, Path, Mapping[str, Any], BaseConfig]) -> BaseConfig:
    ...
    # 8. LLM config is already enforced via .env
...
def load_pipeline_config(
    ...
    *,
    ...
    # LLM config is already enforced via .env
